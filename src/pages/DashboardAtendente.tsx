      ...
      <div className="container mx-auto max-w-7xl space-y-8">
        <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <nav className="flex flex-wrap gap-2 bg-slate-50 p-2 rounded-3xl border border-slate-100">
            <button
              onClick={() => setSecaoAtiva("novos")}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-xs font-black transition-all ${secaoAtiva === "novos" ? "bg-blue-600 text-white shadow-md" : "text-slate-400 hover:text-blue-600"}`}
            >
              <UserPlus size={16} /> NOVOS
            </button>
            <button
              onClick={() => setSecaoAtiva("aprovados")}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-xs font-black transition-all ${secaoAtiva === "aprovados" ? "bg-blue-600 text-white shadow-md" : "text-slate-400 hover:text-blue-600"}`}
            >
              <UserCheck size={16} /> APROVADOS
            </button>
            <button
              onClick={() => setSecaoAtiva("triagem")}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-xs font-black transition-all ${secaoAtiva === "triagem" ? "bg-blue-600 text-white shadow-md" : "text-slate-400 hover:text-blue-600"}`}
            >
              <CalendarClock size={16} /> TRIAGEM
            </button>
            <button
              onClick={() => setSecaoAtiva("reprovados")}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl text-xs font-black transition-all ${secaoAtiva === "reprovados" ? "bg-blue-600 text-white shadow-md" : "text-slate-400 hover:text-blue-600"}`}
            >
              <UserX size={16} /> REPROVADOS
            </button>
          </nav>
        </section>
              ...
              {aprovados.map((beneficiario) => (
                ...
                >
                  ...
                  <div className="space-y-2 mb-6 text-slate-500">
                    <p className="text-xs">
                      <strong>Resp:</strong> {beneficiario.responsavel}
                    </p>
                    <p className="text-xs">
                      <strong>Dentista:</strong> {beneficiario.dentista}
                    </p>
                  </div>
                  <button className="w-full bg-slate-900 text-white text-[10px] font-black py-3 rounded-xl hover:bg-blue-600 transition-all uppercase tracking-widest flex items-center justify-center gap-2">
                    <History size={14} /> VER HISTÓRICO E PRONTUÁRIOS
                  </button>
                </div>
              ))}
            </div>
        ...
        {secaoAtiva === "triagem" && (
          <section className="space-y-6 animate-in fade-in duration-500">
            {triagemPendente.map((item) => (
              <div
                key={item.id}
                className="bg-white p-8 rounded-[2.5rem] border-l-8 border-amber-400 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6"
              >
                <div>
              ...
              {reprovados.map((pessoa) => (
                ...
                >
                  ...
                  <p className="text-sm text-slate-500">
                    Indicar Unidade de Saúde mais próxima do telefone:{" "}
                    {pessoa.contato}
                  </p>
