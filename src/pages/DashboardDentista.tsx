      ...
      <div className="container mx-auto max-w-7xl space-y-10">
        ...
        <section className="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 gap-6">
          <div className="space-y-1">
            <h1 className="text-3xl font-black text-slate-900 leading-none">Olá, Dr. Dentista!</h1>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 space-y-6">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                  <span className="text-2xl">🗓️</span> Sua Agenda
                </h2>
                ...
                <div className="flex gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-100 w-full md:w-auto">
                  <button 
                    ...
                    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${filtroTempo === 'semana' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400'}`}
                  >
                    Semana
                  </button>
                  <button 
                    onClick={() => setFiltroTempo('mes')}
                    className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${filtroTempo === 'mes' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400'}`}
                  >
                    Mês
                  </button>
                </div>
              ...
              <input 
                ...
                className="w-full px-5 py-3 rounded-xl bg-slate-50 border-none focus:ring-2 focus:ring-blue-500 outline-none text-sm transition-all"
              ...
              <div className="space-y-4">
                {agenda.map((item) => (
                  <div key={item.id} className="flex flex-col md:flex-row justify-between items-start md:items-center p-5 rounded-2xl border border-slate-50 hover:bg-slate-50 transition-colors group">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-blue-100 rounded-xl flex items-center justify-center text-xl">🧒</div>
                      <div>
                        <h4 className="font-bold text-slate-900">{item.paciente}</h4>
                        <p className="text-xs text-slate-400 font-medium uppercase tracking-tighter">{item.local}</p>
                      </div>
                    </div>
                    <div className="mt-4 md:mt-0 text-right">
                      <p className="text-sm font-black text-blue-600">{item.data}</p>
                      <p className="text-xs text-slate-400 font-bold">{item.hora}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-slate-900 p-8 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden group">
              <div className="relative z-10 space-y-6">
                ...
                <p className="text-slate-400 text-sm max-w-md">Precisa de insumos para a próxima operação? Solicite o apoio da ONG diretamente por aqui.</p>
          ...
          <div className="space-y-8">
            <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
              <h3 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
                <span className="text-2xl">📝</span> Prontuários Pendentes
              </h3>
              
              <div className="space-y-4">
                {consultasPendentes.map((pendencia) => (
                  <div key={pendencia.id} className="p-5 rounded-2xl bg-orange-50 border border-orange-100 space-y-3">
                    <div>
                      <p className="text-xs font-black text-orange-600 uppercase tracking-widest mb-1">Aguardando Prontuário</p>
                      <h4 className="font-bold text-slate-900">{pendencia.paciente}</h4>
                      <p className="text-[10px] text-slate-400 font-bold uppercase">{pendencia.data} • {pendencia.local}</p>
                    </div>
              ...
              {consultasPendentes.length === 0 && (
                <p className="text-center text-slate-400 text-sm italic py-4">Tudo em dia por aqui! ✨</p>
              )}
            </div>
